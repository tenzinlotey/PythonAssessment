...
def calculate_bill_amount(gems_list, price_list, reqd_gems,reqd_quantity):
    ...
    for gemQuant in zip(reqd_gems,reqd_quantity):
        if gemQuant[0] not in gems_list:
            bill_amount = -1
            return bill_amount
        else:
            priceOfGem = price_list[gems_list.index(gemQuant[0])] 
            totalAmount = priceOfGem * gemQuant[1]
